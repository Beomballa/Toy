...
public class FrontMemberDeliveryAddressService {
    ...
    @Transactional public List<FrontDeliveryAddressResponse> update(long memberNo,long id,FrontDeliveryAddressRequest request){ requireMember(memberNo); var address=addressRepository.findByIdAndMemberNo(id,memberNo).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"배송지를 찾을 수 없습니다.")); address.update(text(request.addressName()),text(request.recipientName()),phone(request.recipientPhone()),text(request.postalCode()),text(request.address1()),blankToNull(request.address2())); if(request.defaultAddress()) setDefault(memberNo,id); return addresses(memberNo); }
