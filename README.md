...
- JDK 21
- MySQL 8.x
- 비어 있거나 기존 데이터가 있는 `new_toy` 데이터베이스

`db/baseline_schema.sql`은 빈 데이터베이스의 전체 테이블·인덱스·제약조건을 생성하며 데이터를 포함하지 않습니다. `scripts/migrate-db.sh`는 기준 스키마와 버전별 SQL의 SHA-256 체크섬을 `schema_migration`에 기록해 신규 환경과 기존 환경에 동일한 스키마를 재현합니다.
