...
- QueryDSL 단일 그룹 조회 기반 문서 일일 통계와 관리자 스냅샷 화면
...
| `batch` | 문서 일일 통계 등 명시적으로 활성화된 예약 작업 실행 | `9091` |
...
공개 콘텐츠 조회수는 브라우저 식별자와 날짜를 기준으로 하루 한 번만 집계됩니다. 배포 전 조회 이벤트 테이블을 먼저 반영해야 하며, 콘텐츠 목록의 최근 읽기 보드는 브라우저 로컬 저장소에 최대 6건만 보관합니다.
