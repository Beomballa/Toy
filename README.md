...
공개 콘텐츠 조회수는 브라우저 식별자와 날짜를 기준으로 하루 한 번만 집계됩니다. 홈의 `이번 주 많이 읽은 콘텐츠`는 최근 7일 조회 이벤트와 순 방문자 수를 기준으로 공개·게시 완료 콘텐츠만 노출합니다. 배포 전 조회 이벤트 테이블을 먼저 반영해야 하며, 콘텐츠 목록의 최근 읽기 보드는 브라우저 로컬 저장소에 최대 6건만 보관합니다.
