...
`front_member_delivery_address`는 회원 배송지를 저장하고 회원·기본 배송지 조회 인덱스를 유지합니다. `front_product_review`와 관련 상태·신고·이력 테이블은 배송 완료 주문 상품에 대한 후기와 운영 검수 상태를 저장합니다.

`front_order_claim`과 `front_order_claim_history`는 배송 완료 주문의 교환·반품 신청, 철회와 운영 처리 이력을 저장합니다. 주문 번호와 회원 번호 복합 인덱스로 회원 이력 조회를, 상태·등록일 인덱스로 관리자 처리 대기 목록을 지원합니다.

`front_password_reset_token`은 고객 비밀번호 재설정 토큰의 해시와 만료 시각을 저장합니다. 이메일 발송 연동은 별도 배포 환경 설정이 필요하므로, 토큰 테이블만으로 외부 메일 전송이 활성화되지는 않습니다.
